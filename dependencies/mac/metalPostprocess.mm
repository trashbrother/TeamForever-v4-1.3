    ...
    if (!attempted) {
        ...
        static NSString *shaderSource =
            @"#include <metal_stdlib>\n"
             ...
             "\n"
             "    float preLuma0 = dot(max(color0.rgb, float3(0.0)), float3(0.2126, 0.7152, 0.0722));\n"
             "    float preLuma1 = dot(max(color1.rgb, float3(0.0)), float3(0.2126, 0.7152, 0.0722));\n"
             "\n"
             "    float2 focusPos = in.uv * 2.0 - 1.0;\n"
             "    float focusRadius = clamp(dot(focusPos, focusPos), 0.0, 1.0);\n"
             "    float edgeFocus = 0.015 * focusRadius;\n"
             "\n"
             "    float spotMix0 = 0.04 * clamp(preLuma0, 0.0, 1.0) + edgeFocus;\n"
             "    float spotMix1 = 0.04 * clamp(preLuma1, 0.0, 1.0) + edgeFocus;\n"
             "\n"
             ...
             "\n"
             "    float darkSupport0 = 0.015 * (1.0 - clamp(luma0, 0.0, 1.0));\n"
             "    float darkSupport1 = 0.015 * (1.0 - clamp(luma1, 0.0, 1.0));\n"
             "    beam0 = max(beam0, darkSupport0);\n"
             "    beam1 = max(beam1, darkSupport1);\n"
             "\n"
             "    float4 color;\n"
